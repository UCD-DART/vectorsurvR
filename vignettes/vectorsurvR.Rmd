...
- start_year: Beginning of year range 
- end_year: End of year range
- arthropod: Type of pools to retrieve: 'tick' , 'mosquito', 'nontick'
- agency_id: Default to NULL returns data for all available agencies, specifying a single agency id will return data just for that agency. This parameter is best used by accounts with access to multiple agencies. 
...
#by index
head(sample_collections[c(2, 10, 8)])

#to save a subset
collections_subset = sample_collections[c(2, 10, 8)]
```
...
sample_collections %>% 
  group_by(surv_year, trap_acronym) %>%
summarise(traps_deployed = sum(num_trap, na.rm=TRUE))%>%
  pivot_wider(names_from = "surv_year", values_from ="traps_deployed", values_fill = 0 )%>%
  kbl(align = "c") %>%
  kable_paper(
    full_width = F,
    html_font = "arial",
