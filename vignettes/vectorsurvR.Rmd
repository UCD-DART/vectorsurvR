...
```{r}
#AbAnOut_L %>%
#  filter(AbundanceType == "Delta") %>%
#  mutate(Change = ifelse(AbundanceCalculation > 0, #"Increase", "Decrease")) %>%
#  ggplot(aes(x = Biweek,
#             y = AbundanceCalculation,
#             fill = Change)) +
#  geom_bar(stat = "identity") +
#  facet_wrap( ~ Species) +
#  labs(x = "Biweek",
...
`plotVectorIndex()` plots the output returned from `getVectorIndex()` for all years in the output data. 
...
- VI: Output from returned `getVectorIndex()`


```{r}
VI = getVectorIndex(
  sample_collections,sample_pools,
  interval = "Week",
  target_disease = "WNV",
  pt_estimate = "mle",
  species = c("Cx pipiens"),
  trap = c("CO2"), wide=FALSE
)
...
table(sample_collections$trap_acronym, sample_collections$surv_year) %>%
  ...
  row_spec(c(2, 3), background = "yellow") %>%
  column_spec(c(3), background = "orange")
```
